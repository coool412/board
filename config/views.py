...
def update(request, id):
  ...
  # select * from article where id = ?
  article = Article.objects.get(id=id)
  if article.user.email != email:     #작성자가 다르면 수정할 수 없어
    return redirect(f'/article/detail/{id}/')  #작성자가 다른 사람이 수정버튼을 눌러도 그 화면에 머무르게 하고싶어
...
def delete(request, id):
  ...
  try:
    # select * from article where id = ?
    article = Article.objects.get(id=id)
    if email != article.user.email:    #작성자가 다르면 삭제 할 수 없어
      return redirect(f'/article/detail/{id}/')  #작성자가 다른 사람이 삭제버튼을 눌러도 그 화면에 머무르게 하고싶어
    article.delete()
